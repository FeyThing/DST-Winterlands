...
HullHealth.OnCollide = function(self, data, ...)
	...
	if damage_alignment > 0.258 then
		...
		if hit_adjacent_speed > 2 then
			...
			if TheWorld.Map:GetTileAtPoint(tpos:Get()) == WORLD_TILES.POLAR_ICE then
				local tx, ty = TheWorld.Map:GetTileCoordsAtPoint(tpos:Get())
				
				TheWorld.components.polarice_manager:DestroyIceAtTile(tx, ty, TUNING.POLAR_ICEGEN_DEFAULT_HOLE_TIME, "ice")
			end
		end
	end
end