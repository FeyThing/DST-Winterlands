...
local function fn()
	...
	local function emit_fn()
		...
		if IsUnderIceCaveAtXZ(abs_px, abs_pz) then
			return
		end
		
		if use_uv_offset then
			local angle = math.random() * 360
			local uv_offset = math.random(0, 7) * 0.125
	...
	local function update_fn()
		if init_effect then
			...
			if IsSpecialEventActive(SPECIAL_EVENTS.WINTERS_FEAST) then
				...
				effect:SetUVFrameSize(0, 0.125, 1)
				effect:SetRotationStatus(0, true)
				use_uv_offset = true
				particle_mult = 2
				effect:SetDragCoefficient(0, 0.85)
			else
...
return Prefab("snow_polar", fn, assets)