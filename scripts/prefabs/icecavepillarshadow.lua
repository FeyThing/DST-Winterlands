local function SpawnShadow(inst, range, no_lightrays)
	...
	if not TheWorld.shadetile_key_to_icecavepillar_shade_id then
		TheWorld.shadetile_key_to_icecavepillar_shade_id = {}
	end
			...
			if ((t * t) + (i * i)) <= range * range then
				...
				if not TheWorld.shadetiles[shadetile_key] or TheWorld.shadetiles[shadetile_key] <= 0 then
					if math.random() < 0.8 then
						TheWorld.shadetile_key_to_icecavepillar_shade_id[shadetile_key] = SpawnIceCavePillarShade(newx, newz)
					elseif not no_lightrays then
						if math.random() < 0.5 then
							local rays = SpawnPrefab("lightrays_canopy") -- TODO: do we want that?
