...
local SnowOver = Class(Widget, function(self, owner, dustlayer)
    ...
    self.bg:GetAnimState():SetBank("snow_over")
    ...
    if owner then
		...
		if owner.GetStormLevel then
			self:FadeTo(owner:GetStormLevel(STORM_TYPES.BLIZZARDSTORM), true)
        end
    end
end)
...
function SnowOver:FadeTo(fadeto, instant)
	if self.owner and self.owner:GetStormLevel(STORM_TYPES.BLIZZARDSTORM) == 0 then
        fadeto = 0
    end
