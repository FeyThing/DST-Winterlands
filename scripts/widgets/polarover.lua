...
local BGCOLOR = { 255/255, 255/255, 255/255, 255/255 }
    ...
    root.OnUpdate = function()
        ...
        if hbars then
            if root.left == nil then
                ...
                root.left:SetPosition(-0.5 * RESOLUTION_X * span, 0)
            end
            ...
            if root.right == nil then
                ...
                root.right:SetPosition(0.5 * RESOLUTION_X * span, 0)
            end
        else
            if root.left then
                root.left:Kill()
        ...
        if vbars then
            if root.top == nil then
                ...
                root.top:SetPosition(0, 0.5 * RESOLUTION_Y * span)
            end
            ...
            if root.bottom == nil then
                ...
                root.bottom:SetPosition(0, -0.5 * RESOLUTION_Y * span)
            end
        else
            if root.top then
                root.top:Kill()
        ...
        elseif hbars then
            local w, h = root.left:GetSize()
            local scalex = (scrnw / (scrnh * aspect * span) - 1) * 0.5 * RESOLUTION_X * span / w
        ...
        elseif vbars then
            ...
            local scaley = (scrnh * aspect / (scrnw * span) - 1) * 0.5 * RESOLUTION_Y * span / h
            root.top:SetScale(scalex, scaley)
            root.bottom:SetScale(scalex, scaley)
        end
    end
...
local PolarOver = Class(Widget, function(self, owner, dustlayer)
    self.owner = owner
    Widget._ctor(self, "PolarOver")
    self:UpdateWhilePaused(false)

    self:SetClickable(false)

    self.minscale = 0.9 -- Min scale supported by art size
    ...
    self.bg:GetAnimState():SetBank("polarstorm_over")
    self.bg:GetAnimState():SetBuild("polarstorm_over")
    ...
    if owner then
		...
		if owner.GetStormLevel then
			self:FadeTo(owner:GetStormLevel(STORM_TYPES.POLARSTORM), true)
        end
    end
end)

function PolarOver:BlindTo(blindto, instant)
...
function PolarOver:FadeTo(fadeto, instant)
	if self.owner and self.owner:GetStormLevel(STORM_TYPES.POLARSTORM) == 0 then
        fadeto = 0
    end
...
function PolarOver:ApplyLevels()
    self.alpha = math.max(0, self.fade * 1.5 - 0.5) * self.blind
    ...
    if self.fade > 0 then
        local k = Lerp(1, 0.7, self.brightness)
        ...
        if not self.dust.shown then
            ...
            TheFocalPoint.SoundEmitter:SetParameter("sandstorm", "intensity", self.blindto < 1 and 0 or 0.5)
        else
...
function PolarOver:OnUpdate(dt)
